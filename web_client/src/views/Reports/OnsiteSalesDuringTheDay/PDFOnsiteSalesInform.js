...
import dataChartToURLv2 from 'utils/dataChartToURLv2';
import styles from '../styles';
import dateText from 'utils/DateFormat/dateText';

// Create Document Component
export default function MyDocument ({title, description, user, chart, salesData }) {
  return (
    <>
      <Document>
        <Page size="A4" style={styles.page} title={dateFormat(new Date()) + '- VentasEnElLocal - ' + title + ' - ' + description} author={'Heladería y cafetería - La Aldeana'}>
        <View style={styles.sectionFace} fixed>
          ...
          <View style={styles.section}>
            ...
            <Image style={styles.image} src={dataChartToURLv2(chart)}></Image>
            ...
            {Object.values(salesData).map((dataSalePerPayType) => {
                ...
                return (
                    ...
                    <View style={styles.row}>
                        <View style={styles.col}>
                        <Text style={styles.text}>{dataSalePerPayType[0].name}</Text>
                        </View>
                        <View style={styles.col}>
                        <Text style={styles.money}>{dataSalePerPayType.length + ' uds.'}</Text>
                        </View>
                        <View style={styles.col}>
