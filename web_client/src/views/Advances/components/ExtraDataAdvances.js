...
export default function ExtraDataAdvances(props) {
  ...
  const [isValidClassAmountTotal, setIsValidClassAmountTotal] =
  ...
  const [maxFirstMonth, setMaxFirstMonth] = useState(
    formattedDate(new Date(), 6)
  );
  ...
  useEffect(() => {
    if (props.data.reading || props.data.editing) {
      Axios.get(
        PORT() +
          `/api/installments?dniEmployee=${props.data.nroDNI}&date=${props.data.date}`
      )
        .then((response) => {
          let aux = 0;
          response?.data.map((installments) => {
            if (installments.pay) aux += 1;
          });
          setPayedCuotes(aux);
  ...
  useEffect(() => {
    if (!employee) {
      ...
      if (data.dniEmployee !== employee) {
        employees?.forEach((e) => {
          if (e.dni === employee && e.advance > 3)
            warningMessage(
              "Atención",
              "El empleado seleccionado ya posee más de 3 adelantos registrados.",
              "warning"
            );
        });
      }
  ...
  useEffect(() => {
    if (data.date !== inputDate.current.value) {
      ...
      props.load(data);
      if (
        parseInt(firstMonth.slice(0, -5)) === parseInt(data.date.slice(0, -5))
      ) {
        if (
          parseInt(firstMonth.slice(5, -3)) < parseInt(data.date.slice(5, -3))
        ) {
      ...
      <div className="formRow d-flex">
        ...
        <div className="form-control-input">
          <input
            ...
            readOnly={props.data.reading || props.data.editing}
        ...
        <BeShowed show={props.data.reading || props.data.editing}>
          <div className="form-control-input">
            <input
              ...
              maxLength="80"
              ref={inputEmployee}
              defaultValue={props.data.fullName && props.data.fullName}
            />
          </div>
        </BeShowed>
        <BeShowed show={!props.data.reading && !props.data.editing}>
          <UploadByName
            ...
            placeholder="Ingrese el nombre del empelado que busca..."
            maxLength="80"
          />
        </BeShowed>
      </div>
      <div className="formRow d-flex">
        ...
        <div className="form-control-input">
          <input
            ...
            style={{ maxWidth: "100em", marginLeft: "auto" }}
            id="firstMonth"
            readOnly={props.data.reading}
            type="month"
            ref={inputFirstMonth}
            onChange={onChangeFirstMonth}
            min={date !== "null" ? date.slice(0, -3) : startDate.slice(0, -3)}
            max={maxFirstMonth.slice(0, -3)}
          />
        </div>
      </div>
      <div className="formRow">
        ...
        <div className="form-control-input d-flex">
          <span
            ...
            style={{ width: "2em", marginLeft: "auto" }}
          >
            $
          </span>
          <input
            className={isValidClassAmountTotal}
            readOnly={props.data.reading || !inputFirstMonth.current?.value}
            style={{ width: "98em" }}
            id="amountTotal"
            type="number"
            ref={inputAmountTotal}
            onChange={handleAmountTotal}
            min={months > minTotal ? months : minTotal}
            placeholder="Ingrese monto..."
            onKeyDown={(e) => validateFloatNumbers(e)}
            onInput={(e) => validate(e)}
            defaultValue={props.data.amount ? props.data.amount : null}
          />
        </div>
      </div>
      ...
      <div className="formRow">
        <div className="form-control-label">
          <label>
            ...
            {amountTotal - data.pay > 0
              ? amountTotal - data.pay
              : data.amount - data.pay > 0
      ...
      <div className="formRow d-flex">
        ...
        <div className="form-control-input">
          <input
            ...
            min={minCuotes}
            max={18 > amountTotal ? amountTotal : 18}
            placeholder="Ingrese cantidad de cuotas..."
            onKeyDown={(e) => validateFloatNumbers(e)}
            onInput={(e) => validateMonts(e)}
            defaultValue={data.months ? data.months : null}
          />
