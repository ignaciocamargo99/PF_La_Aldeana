...
const validateHoursEgressEntry = (inputDateEntry, inputDateEgress, dateEntry, dni, dateEgress, assistance, id_assistance, editing, PORT) => {
    ...
    if (!editing) {
        ...
        // FECHAS DISTINTAS!!! 
        assistancesWithDateEntryDateEgress1 = assistance.find((employee) =>
            employee.date_egress
            &&
            (
                (
                    (moment(employee.date_entry).add(hours, 'hours').format('HH:mm') <= dateEntry)
                ...
                (
                    (moment(employee.date_entry).add(hours, 'hours').format('HH:mm') <= dateEgress)

                    &&

                    (dateEgress <= moment(employee.date_egress).add(hours, 'hours').format('HH:mm'))
                )
            )
        ...
        assistancesWithDateEntryDateEgress2 = assistance.find((employee) =>

            employee.date_egress
            &&
            (
                ((dateEntry <= moment(employee.date_entry).add(hours, 'hours').format('HH:mm')) && (moment(employee.date_entry).add(hours, 'hours').format('HH:mm') <= dateEgress))

                &&

                ((dateEntry <= moment(employee.date_egress).add(hours, 'hours').format('HH:mm')) && (moment(employee.date_egress).add(hours, 'hours').format('HH:mm') <= dateEgress))
            )
