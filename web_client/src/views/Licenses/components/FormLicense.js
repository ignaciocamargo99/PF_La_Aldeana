...
const FormLicense = (props) => {
  ...
  useEffect(() => {
    if (props.action !== "Registrar") {
      ...
      Axios.get(`${PORT()}/api/employees/${props.license.dni}`).then(
        (response) => {
      ...
      if (props.action === "Ver") {
        ...
        props.licenses.forEach((license) => {
          ...
          if (
            license.dni === props.license.dni &&
            dateFinish.getTime() <= licenseDateInit.getTime() &&
            (dateFinishLastLicense === "" ||
              dateFinish.getTime() > dateFinishLastLicense.getTime())
          ) {
            dateFinishLastLicense = dateFinish;
          }
          if (
            license.dni === props.license.dni &&
            dateInit.getTime() >= licenseDateFinish.getTime() &&
            (dateInitFutureLicense === "" ||
              dateInit.getTime() < dateInitFutureLicense.getTime())
          ) {
            dateInitFutureLicense = dateInit;
          }
        });
        dateInitRef.current.min =
          dateFinishLastLicense !== ""
            ? formattedDate(dateFinishLastLicense)
            : formattedDateWithDay(new Date(dateInitRef.current.value), 0, -15);
        dateInitRef.current.max =
          dateInitFutureLicense !== ""
            ? formattedDate(dateInitFutureLicense)
            : dateInitFutureLicense;
        dateFinishRef.current.min =
          dateFinishLastLicense !== ""
            ? formattedDate(dateFinishLastLicense)
            : formattedDateWithDay(new Date(dateInitRef.current.value), 0, -15);
        dateFinishRef.current.max =
          dateInitFutureLicense !== ""
            ? formattedDate(dateInitFutureLicense)
            : dateInitFutureLicense;
      }
  ...
  const employeesUpload = async () => {
    if (dateInitRef.current && dateInitRef.current.value !== "") {
      ...
      await Axios.get(
        `${PORT()}/api/employeesDate/${dateInitRef.current.value}`
      )
        .then((response) => {
  ...
  const findLicensedEmployees = (licenses, dateInit, dateFinish) => {
    ...
    licenses.forEach((license) => {
      let licenseDateInitNumber = new Date(
        dateBDToString(license.date_init, "En")
      ).getTime();
      let licenseDateFinishNumber = new Date(
        dateBDToString(license.date_finish, "En")
      ).getTime();
      if (
        (dateInitNumber <= licenseDateInitNumber &&
          licenseDateInitNumber <= dateFinishNumber) ||
        (dateInitNumber <= licenseDateFinishNumber &&
          licenseDateFinishNumber <= dateFinishNumber) ||
        (licenseDateInitNumber < dateInitNumber &&
          licenseDateFinishNumber > dateFinishNumber)
      ) {
  ...
  const onChangeDates = () => {
    let aux = calculateDiferenceDays(
    ...
    setDays(aux);
    if (props.action === "Registrar") {
      let employeesAux = findLicensedEmployees(
        props.licenses,
        dateInitRef.current.value,
        dateFinishRef.current.value
      );
  ...
  const registerLicense = async () => {
    const registrationConfirmed = (
      await defaultQuestionSweetAlert2(`¿Registrar nueva licencia?`)
    ).isConfirmed;
    if (registrationConfirmed) {
      loadingMessage("Registrando nueva licencia");
      Axios.post(`${PORT()}/api/licenses`, {
        date_init: dateInitRef.current.value,
        date_finish: dateFinishRef.current.value,
        dni_employee: employee.dni,
  ...
  const editLicense = async () => {
    const editionConfirmed = (
      await defaultQuestionSweetAlert2("¿Confirmar cambios?")
    ).isConfirmed;
    if (editionConfirmed) {
  ...
  const actionNotOK = () => {
    if (errorDateInit) {
      warningMessage(
        "Atención",
        "Se debe cargar una fecha de inicio",
        "warning"
      );
    } else if (errorDateFinish) {
      warningMessage(
        "Atención",
        "Se debe cargar una fecha de finalización",
        "warning"
      );
      ...
      warningMessage(
        "Atención",
        "Se debe cargar un motivo de la licencia",
        "warning"
      );
    }
  };
  ...
  return (
    <>
      <Breadcrumb
        parentName="Licencias"
        icon={faUserFriends}
        parentLink="licenses"
        currentName={`${props.action} licencia`}
      />
      <div style={{ display: "none" }}>
        {(document.title = `${props.action} Licencia`)}
      </div>
      <div className="viewTitleBtn">
        <h1>
          {props.action} licencia:{" "}
          {props.action !== "Registrar"
            ? props.license?.id_license +
              " - " +
              props.license?.name +
              " " +
              props.license?.last_name
            : ""}
        </h1>
      </div>
      <div className="container">
        ...
        <BeShowed show={!showSpinner && props.action === "Registrar"}>
          <div className="formRow justify-content-center">
            <CardEmployees
              ...
              setEmployeesStart={setEmployeesStart}
              employees={employees}
              employeesStart={employeesStart}
              searchState={searchState}
              licensedEmployees={licensedEmployees}
            />
          </div>
        </BeShowed>
        ...
        <BeShowed show={props.action === "Registrar"}>
          <Buttons
            ...
            label="Registrar"
            actionNotOK={actionNotOK}
            actionOK={registerLicense}
            actionCancel={() => {
              resetStates(false);
            }}
          />
        ...
        <BeShowed show={props.action === "Ver"}>
          <button
            className="btn btn-light sendOk offset-sm-11"
            onClick={() => {
              comeBack(false);
            }}
          >
